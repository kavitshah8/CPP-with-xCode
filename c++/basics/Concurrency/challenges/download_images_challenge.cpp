...
// learnings:
// can't really use threads as we will need the return value
// i/o bound problem, so  increasing process does not help much
// wifi usage increases in the parallel version
...
size_t parallel_image_downloader(int num_images) {
    size_t total_bytes = 0;
    std::list<std::future<size_t>> image_size_total;
    for (size_t i = 0; i < num_images; i++) {
        image_size_total.push_back(std::async(std::launch::async, sequential_image_downloader, i));
    }
    for (size_t i = 0; i < num_images; i++) {
        total_bytes += image_size_total.front().get();
        image_size_total.pop_front();
    }
